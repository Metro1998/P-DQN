...
class Replay_buffer(object):
    """ Replay buffer to store past experiences taht the agent can then use for training data. """
    ...
    def push(self, states, actions, rewards, next_states, dones):
        """ Pushs experience(s) into replay buffer. """
        if type(dones) == list:
            assert type(dones[0]) != list, "A done shouldn't be a list"
            experiences = [self.experience(state, action, reward, next_state, done)
                           for state, action, reward, next_state, done in
                           zip(states, actions, rewards, next_states, dones)]
            self.memory.extend(experiences)  # List to lit
        else:
    ...
    def separate_out_data_types(self, experiences):
        """ Puts the sampled experiences into correct format for a PyTorch neural network. """
