...
class SumTree(object):
    ...
    elements with zero values.  # TODO
    ...
    def __init__(self, capacity):
        ...
        :param capacity: int, the maximum number of elements that can be stored in this data structure.
        :raise ValueError: If requested capacity is not positive.
        """
        ...
        self.nodes = []
        tree_depth = int(math.ceil(np.log(capacity)))
        level_size = 1
    ...
    def sample(self, query_value=None):
        ...
        :raise  Exception: If the sum tree is empty (i.e. its node values sum to 0), or if
    ...
    def stratified_sample(self, batch_size):
        ...
        :param batch_size: int, the number of strata to use.
        :return: list of batch_size elements sampled from the sum tree.
        :raise Exception: If the sum tree is empty (i.e. its node values sum to 0).
        """
        if self._total_priority() == 0.0:
            raise Exception('Cannot sample from an empty sum tree.')

        bounds = np.linspace(0., 1., batch_size)
    ...
    def set(self, node_index, value):
        ...
        :param value:float, the value which we assign to the node. This value must be nonnegative.
        Setting value = 0 will cause the element to never be
        sampled.
        :raise ValueError: If the given value is negative.
        """
        if value < 0.0:
            raise ValueError('Sum tree values should be nonnegative. Got {}'.
                             format(value))

        self.max_recorded_priority = max(value, self.max_recorded_priority)  # TODO
